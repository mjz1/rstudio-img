.PHONY: help build build-4.3 build-4.4 build-4.5 build-4.6 build-all up down restart logs clean lint pr-validate
...
help:
	...
	@echo ""
	@echo "Build commands:"
	@echo "  make build        - Build image with default R version (4.6)"
	...
	@echo "  make build-4.6    - Build image with R 4.6"
	@echo "  make build-all    - Build images for R 4.3, 4.4, 4.5, 4.6"
...
# Build commands
build:
	docker build --platform linux/amd64 --build-arg R_VERSION=4.6 -t rstudio-local:4.6 .
